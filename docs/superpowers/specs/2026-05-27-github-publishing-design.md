...
При смене `groupId` пакеты Java остаются прежними (`io.github.nikolaynn.docengine.*`) — Maven groupId и Java package — независимые координаты. В будущем при желании можно переименовать пакеты, но это отдельная задача и не блокирует первый релиз. Этот пункт фиксирует: **рефакторинг Java-пакетов в этот спек не входит**.
